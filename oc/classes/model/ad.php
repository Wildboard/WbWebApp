...
// d($title);
	...
	fwrite($fh, "In save_image with ".print_r($image,true)."\n");
	fclose($fh);
	...
	$fh = fopen('/tmp/grisha.log', 'a');	   
	fwrite($fh, "EMPTY!!!: ".print_r($image,true)."\n");
	fclose($fh);
	...
	$fh = fopen('/tmp/grisha.log', 'a');	   
	fwrite($fh, "XXXX Image: ".print_r($image,true)."\n");
	//	    $width=180;
	//	    $height =180;
	    ...
	    fwrite($fh, "1. $height : $height_thumb");            
	    ...
	    $save_result = Upload::save($image, NULL, $directory);
   	    fwrite($fh, "\nCalling save($image, NULL, $directory) ==> $save_result\n");            

            if ($file = $save_result)
            {
                $filename_thumb     = 'thumb_'.$seotitle.'_'.$counter.'.jpg';
                $filename_original  = $seotitle.'_'.$counter.'.jpg';
   	    fwrite($fh, "Thumb $filename_thumb, orig $filename_original\n");
                /*WATERMARK*/
